...
const Page = () => {
    ...
    return <Content title="Update Teacher" error={error} isLoading={loading} >
        {data && <Create fullname={data.user.fullname} department={data.departments.map(({ id, name }) => ({ id, label: name }))} isLoading={loading || updateLoading} onSubmit={(payload) => { updateRequest(id, { ...payload }) }} />}
    </Content >
}
export default Page;