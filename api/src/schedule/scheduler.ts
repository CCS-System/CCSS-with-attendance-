export const OR = (arr1: boolean[], arr2: boolean[], arr3: boolean[]) => {
...
export function CheckForRepeat(startIndex: number, originalArray: boolean[], valueToCheck: any) {
    var repeatCount = 1;
...
export const findAllPossibleSlots = (duration: number, avalibility: boolean[]) => {
...
export const findSlots = (duration: number, avalibility: boolean[]) => {
    const requiredSlots = duration * 2;
    const reservedSlots = [9, 10]
    for (let i = 0; i < avalibility.length; i++) {
        const numberOfRepeats = CheckForRepeat(i, avalibility, false);
...
export const generateForSection = (classes: Record<any, any>[], section: Record<any, any>, teacherLookup: Record<any, any>, classroomLookup: Record<any, any>[]) => {
    const schedules = [];
    shuffle(classes);
...
export const runScheduler = (e: Record<any, any>) => {
